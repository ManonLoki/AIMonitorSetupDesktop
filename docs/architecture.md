...
- Profile 按“设备 ID + AI 工具”隔离保存显示位置及四种显示状态，不保存
  ...
  阻止其余设备，也不延迟其余设备的下一次状态更新。设备快照 revision 推进时，
  Hook worker 会比较设备 ID、基地址与 API 路径；仅向新上线、重新上线或地址变化且
  已配置对应 Profile 的设备，以内部 `DeviceOnlineReplay` 转换重放各状态机当前的
  聚合 Display。原本在线的设备不重复接收，Released 和逐事件直通工具不重放。
...
- listener、状态机 worker 与设备投递 workers 分为两个阶段：最小 Hook 事件先进入
  ...
  请求仍不可撤回。
- 设备上线重放由同一个状态机 worker 消费在线快照 revision 后执行，不建立第二份
  设备状态事实。`DeviceOnlineReplay` 不计入收到、待处理或时序抑制次数；实际成功
  与失败仍计入转发/失败统计并作为最近显式 Display 或错误展示。重放同样经过目标
  级 latest-wins 队列，因此与同时到达的 Asking、Stop 等真实转换保持最终状态一致。
